...
// Make sure Env is defined, potentially by importing from worker-configuration.d.ts if not global
// Ensure Env contains: DB: D1Database;
...
// GET /readings - Fetch all sensor readings
app.get('/readings', async (c) => {
	const db = c.get('db');
	try {
		const allReadings = await db.select().from(schema.sensorReadings).all();
		return c.json({ success: true, data: allReadings });
	} catch (e: any) {
		console.error("Error fetching readings:", e);
		return c.json({ success: false, error: `Failed to fetch readings: ${e.message}` }, 500);
	}
});
...
app.post('/readings', async (c) => {
	...
	try {
		...
		const result = await db.insert(schema.sensorReadings).values({
